...
from src.services import (
    ...
    generate_tokens, verify_token, REFRESH_TOKEN_TYPE
)
from src.validators import (
    check_email_uniqueness, validate_password, get_current_user, get_current_active_user
)
...
async def create_user(
    ...
    if existing_user:
        ...
        grace_period_end = existing_user.deactivated_at + timedelta(
            days=settings.DEACTIVATION_GRACE_PERIOD_DAYS
        )
...
async def reactivate_account(
    ...
    if user is None or user.is_active:
        raise HTTPException(status_code=404, detail="No deactivated account found for this email")
...
async def change_password(
    ...
    if not current_user.must_change_password:
        ...
        is_valid = validate_password(
            payload.current_password, current_user.hashed_password.encode("utf-8")
        )
        if not is_valid:
